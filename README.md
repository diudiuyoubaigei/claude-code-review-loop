...
- Windows：使用 PowerShell 脚本 `scripts/dispatch-claude.ps1`。
- macOS / Linux：使用 shell 脚本 `scripts/dispatch-claude.sh`，无需安装 PowerShell。

两个脚本行为一致：都接受 prompt 或 prompt 文件、可选工作目录、权限模式、超时和重试次数。
  ...
  claude-code-review-loop/
    ...
    skills/
      claude-code-review-loop/
        ...
        scripts/dispatch-claude.sh
        scripts/validate-dispatch.sh
```
...
```bash
"./plugins/claude-code-review-loop/skills/claude-code-review-loop/scripts/dispatch-claude.sh" \
  -d "/path/to/your/repo" \
  -p "修复这个问题，并运行相关验证" \
  -t 7200 \
  -r 1 \
...
macOS/Linux 上可以运行内置验证脚本，确认 shell 调度脚本的参数校验、重试、prompt 文件和工作目录行为：

```bash
./plugins/claude-code-review-loop/skills/claude-code-review-loop/scripts/validate-dispatch.sh
```
